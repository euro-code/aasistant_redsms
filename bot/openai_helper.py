...
GPT_4_MODELS = ("gpt-4", "gpt-4-0613", "gpt-4-turbo-preview")
GPT_4_32K_MODELS = ("gpt-4-32k", "gpt-4-32k-0613")
GPT_4_VISION_MODELS = ("gpt-4o",)
GPT_4_128K_MODELS = ("gpt-4-1106-preview", "gpt-4-0125-preview", "gpt-4-turbo-preview", "gpt-4-turbo", "gpt-4-turbo-2024-04-09")
GPT_4O_MODELS = ("gpt-4o",)
GPT_ALL_MODELS = GPT_4_MODELS + GPT_4_32K_MODELS + GPT_4_VISION_MODELS + GPT_4_128K_MODELS + GPT_4O_MODELS
...
def are_functions_available(model: str) -> bool:
    ...
    """
    # Deprecated models
    if model in ("gpt-4-0314", "gpt-4-32k-0314"):
        return False
    # Stable models will be updated to support functions on June 27, 2023
    if model in ("gpt-4", "gpt-4-32k", "gpt-4-1106-preview", "gpt-4-0125-preview", "gpt-4-turbo-preview"):
        return datetime.date.today() > datetime.date(2023, 6, 27)
    # Models gpt-4-turbo and gpt-4-turbo-preview
    if model in ("gpt-4-turbo", "gpt-4-turbo-2024-04-09"):
        return True
    # Specific model that does not support functions
...
def localized_text(key, bot_language='ru'):
    """
    ...
    try:
        ...
        logging.warning(f"No translation available for key '{key}'")
        # Fallback to the key itself if the translation is not available
        return key
...
class OpenAIHelper:
    ...
    async def get_chat_response(self, chat_id: int, query: str) -> tuple[str, str]:
        ...
        """
        # Send the initial request to GPT-4o
        response = await self.__common_get_chat_response(chat_id, query, model=GPT_4O_MINI)
    ...
    async def __common_get_chat_response(self, chat_id: int, query: str, model: str, stream=False):
        """
    ...
    def __count_tokens(self, messages) -> int:
        ...
        if model in GPT_4_MODELS + GPT_4_32K_MODELS + GPT_4_VISION_MODELS + GPT_4_128K_MODELS + GPT_4O_MODELS:
    ...
    async def get_chat_response_stream(self, chat_id: int, query: str):
        """
        Gets a full response from the GPT model in a streaming manner.
        :param chat_id: The chat ID
        :param query: The query to send to the model
        :return: Yields the answer from the model in chunks
        """
        # Determine the model to use based on the query
        model = await self.__determine_model(query)

        # Send the initial request to the determined model
        response = await self.__common_get_chat_response(chat_id, query, model=model, stream=True)
    ...
    async def __determine_model(self, query: str) -> str:
        """
        Determines the appropriate model to use for the given query.
        :param query: The query to analyze
        :return: The name of the model to use
        """
        # Определите логику выбора модели
        return "gpt-4o"
    ...
    #     _, last day of month = monthrange(today.year, today.month)
    #     last day = date(today.year, today.month, last_day_of_month)
    #     params = {
